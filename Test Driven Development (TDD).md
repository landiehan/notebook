...
Expressive呢就是说我们的测试更易读，更具有描述性。在verify阶段，借助Node.js提供的`assert`库，可以让verify这一部分的代码更expressive。

### `assert`方法们

`assert`这个库里面的函数们会比较值，然后根据需要抛出异常。
...
**注意**：第二行更*expressive*，读者不用再去分析整个表达式，看到`equal`就知道了这个测试是在验证相等。

#### `assert.strictEqual`

注意👆这个例子里面，是`==`，比如exp1是`1`，exp2是`'1'`。

就是：

- `assert.equal()`执行`==`比较。
- `assert.strictEqual()`执行`===`比较。
...
👆这两个assertions都会抛出错误❌，因为在JavaScript中，不同的对象，即便里面的属性和值都相等，也是不相等的。

`assert.deepEqual()`这个方法呢，比较每一个对象里面的值，使用`==`方法。

#### `assert.deepEqual` - 2

👆上面这个例子呢，使用`==`来比较两个对象里面的值。数组呢也是对象，所以`deepEqual()`也可以比较数组的值，使用`==`方法。

#### 总结

这一部分呢学习了Node的`assert`库。这个库里还有很多方法，比如`.notEqual()`，可以去文档里看看：[Node.js的文档](https://nodejs.org/api/assert.html)。
